// 期货搜索 - 东方财富 suggest API (type=30 期货)
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // 方案1：使用 suggest API type=30 (期货)
  const token = 'D43BF722C8E33BDC906FB84D85E326E8'; // 同股票搜索
  ...
  try {
    ...
    const list = arr.map((d: any) => {
      const code = d.Code || '';
      const mkt = d.MarketinGbk; // 期货市场代码
      return {
        code: 'f_' + code,
        display_code: code,
        name: d.Name || '',
  ...
  // 方案2：回退 futsseapi 列表接口 (token 可能失效)
