...
// api/_handlers/_shared/response.ts
...
async function responseToText(r) {
  ...
  if (charset === "gbk" || charset === "big5" || import_iconv_lite.default.encodingExists(charset)) {
    try {
      return import_iconv_lite.default.decode(bytes, charset);
    } catch {
    }
  }
  try {
...
function decodeBytes(bytes, contentType) {
  ...
  if (import_iconv_lite.default.encodingExists(charset)) {
    try {
      return import_iconv_lite.default.decode(bytes, charset);
    } catch {
    }
...
function nativeGetText(fullUrl, headers, redirects = 3) {
...
async function fetchText(fullUrl, referer) {
  const headers = { "User-Agent": DEFAULT_UA, ...referer ? { Referer: referer } : {} };
  ...
  for (const target of targets) {
    try {
      if (typeof fetch === "function") {
        ...
        clearTimeout(t);
        if (r && typeof r.arrayBuffer === "function") return await responseToText(r);
      }
    } catch {
    }
...
async function httpGetJson(fullUrl, referer) {
...
function nativePost(fullUrl, body, headers) {
  return new Promise((resolve) => {
    try {
      const u = new URL(fullUrl);
      const mod = u.protocol === "https:" ? import_node_https.default : import_node_http.default;
      const req = mod.request(u, { method: "POST", headers: { ...headers, "Content-Length": Buffer.byteLength(body) } }, (res) => {
        const chunks = [];
        res.on("data", (c) => chunks.push(c));
        res.on("end", () => resolve(decodeBytes(Buffer.concat(chunks), res.headers["content-type"] || null)));
        res.on("error", () => resolve(""));
...
async function httpPostJson(fullUrl, body, referer) {
  const payload = typeof body === "string" ? body : JSON.stringify(body);
  const headers = { "User-Agent": DEFAULT_UA, "Content-Type": "application/json", ...referer ? { Referer: referer } : {} };
  try {
    if (typeof fetch === "function") {
      const ctrl = new AbortController();
      const t = setTimeout(() => ctrl.abort(), 15e3);
      const r = await fetch(fullUrl, { method: "POST", headers, body: payload, signal: ctrl.signal });
      clearTimeout(t);
      const text2 = await responseToText(r);
      if (text2) return JSON.parse(text2);
    }
  } catch {
  }
...
async function httpsGetText(fullUrl, referer) {
...
function toTencentCode(code) {
  return toSinaCode(code);
}
function toCleanCode(sinaCode) {
...
function tencentTextToDiff(text) {
  return text.split("\n").filter((l) => l.trim()).map((line) => {
    ...
    const result = {
      ...
      // 五档盘口
      buy1: parseFloat(p[9]) || 0,
      buy1vol: parseInt(p[10]) || 0,
      buy2: parseFloat(p[11]) || 0,
      buy2vol: parseInt(p[12]) || 0,
      buy3: parseFloat(p[13]) || 0,
      buy3vol: parseInt(p[14]) || 0,
      buy4: parseFloat(p[15]) || 0,
      buy4vol: parseInt(p[16]) || 0,
      buy5: parseFloat(p[17]) || 0,
      buy5vol: parseInt(p[18]) || 0,
      sell1: parseFloat(p[19]) || 0,
      sell1vol: parseInt(p[20]) || 0,
      sell2: parseFloat(p[21]) || 0,
      sell2vol: parseInt(p[22]) || 0,
      sell3: parseFloat(p[23]) || 0,
      sell3vol: parseInt(p[24]) || 0,
...
async function handler(req, res) {
  ...
  const url2 = `https://push2.eastmoney.com/api/qt/clist/get?pn=1&pz=200&po=1&np=1&ut=${ut}&fltt=2&invt=2&wbp2u=|0|0|0|web&fid=f3&fs=${encodeURIComponent(fs)}&fields=f12,f14,f2,f3,f4,f5,f6,f8,f15,f16,f17,f18`;
  const r = await httpGetJson(url2, "https://quote.eastmoney.com/center/gridlist.html");
...
// api/_handlers/em-news-search.ts
...
async function handler5(req, res) {
  ...
  if (!ranks.length) diff.forEach((x, i) => {
    x.rank = i + 1;
    x.hisRc = 0;
  });
...
// api/_handlers/intraday.ts
function parseMinuteRows(mdata) {
  let prevVol = 0;
  return (mdata || []).map((d) => {
...
function deriveTicks(minutes, preClose) {
  const ticks = [];
...
async function handler7(req, res) {
  ...
  if (["5m", "15m", "30m", "60m"].includes(period)) {
    ...
    const list = Array.isArray(stripJsonp(r2)) ? stripJsonp(r2) : [];
    const rows2 = list.map((d) => `${d.day || ""},${d.open || 0},${d.close || 0},${d.high || 0},${d.low || 0},${d.volume || 0}`);
    json(res, 200, { data: { klines: rows2 } });
    return;
  }
  ...
  const rows = (data[key] || []).map((row) => `${row[0]},${row[1]},${row[2]},${row[3]},${row[4]},${row[5] || 0}`);
...
async function handler11(req, res) {
  ...
  let volumeRatio = 0;
  try {
    const lastT = /* @__PURE__ */ new Date();
    do {
      lastT.setDate(lastT.getDate() - 1);
    } while (lastT.getDay() === 0 || lastT.getDay() === 6);
  ...
  try {
    ...
    const fb = {};
    (r3?.data?.fenbu || []).forEach((o) => {
      const k = Object.keys(o || {})[0];
      if (k !== void 0) fb[k] = Number(o[k]) || 0;
    });
    const sumRange = (a, b) => {
      let s = 0;
      for (let k = a; k <= b; k++) s += fb[String(k)] || 0;
      return s;
    };
    dist.zt = fb["11"] || 0;
    dist.g5 = sumRange(6, 10);
    dist.g1 = sumRange(2, 5);
    dist.g0 = fb["1"] || 0;
    dist.flat = fb["0"] || 0;
    dist.d0 = fb["-1"] || 0;
    dist.d1 = sumRange(-5, -2);
    dist.d5 = sumRange(-10, -6);
    dist.dt = fb["-11"] || 0;
  ...
  try {
    ...
    (r4?.data?.diff || []).forEach((row) => {
      mainInflow += Number(row?.f62) || 0;
    });
  } catch {
  }
  ...
  try {
    ...
    if (ztPool.length > 0) {
      const ztCodes = ztPool.slice(0, 30).map((x) => toTencentCode(x.c || "")).join(",");
      if (ztCodes) {
...
async function handler12(req, res) {
  ...
  const rawCodes = isOverview ? MARKET_INDEX_CODES2 : getQuery(req, "codes");
...
async function handler13(req, res) {
  ...
  const ulistFields = "f7,f9,f20,f21,f23";
  ...
  const diff = [{
    ...
    buy5vol: tq.buy5vol,
    sell1: tq.sell1,
    sell1vol: tq.sell1vol,
    sell2: tq.sell2,
    sell2vol: tq.sell2vol,
    sell3: tq.sell3,
    sell3vol: tq.sell3vol,
    sell4: tq.sell4,
...
// api/_handlers/sector-flow-rank.ts
var SINA_NAME_MAP = {
  new_dzxx: "\u7535\u5B50\u4FE1\u606F",
  new_dzqj: "\u7535\u5B50\u5668\u4EF6",
  new_ysjs: "\u6709\u8272\u91D1\u5C5E",
...
// api/_handlers/stock-profile.ts
function fmtHoldNum(v) {
  const n = Number(v || 0);
...
async function handler19(req, res) {
  ...
  if (sub === "company" || sub === "essential") {
    ...
    const jsonData = typeof r === "string" ? (() => {
      try {
        return JSON.parse(r);
      } catch {
        return null;
  ...
  if (sub === "holder") {
    ...
    if (!list.length) {
      json(res, 200, { data: { items: [{ label: "\u63D0\u793A", value: "\u6682\u65E0\u80A1\u4E1C\u6570\u636E" }] } });
      return;
    }
...
// api/_handlers/stock-fflow-day.ts
...
async function fallbackFromSina(code, lmt) {
  const daima = toSinaCode(code);
  if (!daima) return [];
  ...
  const arr1 = Array.isArray(lscjfb) ? lscjfb : [];
  const arr2 = Array.isArray(zjlrqs) ? zjlrqs : [];
...
async function handler20(req, res) {
  ...
  const url2 = `https://push2his.eastmoney.com/api/qt/stock/fflow/daykline/get?lmt=${lmt}&klt=101&secid=${secid}&fields1=${fields1}&fields2=${fields2}&ut=b2884a393a59ad64002292a3e90d46a5`;
  const r = await httpGetJson(url2, "https://data.eastmoney.com/");
...
// api/_handlers/stock-finance.ts
function fmtAmt(v) {
  const n = Number(v || 0);
  ...
  if (!diff.length) {
    ...
    for (const d of arr) {
      ...
      diff.push({
        ...
        f62: net,
        // 净流入（真实）
        f66: Number(d.r0_net || 0),
        // 主力净流入（真实，r0_net）
        f72: Number(d.r0_ratio || 0) * 100,
        // 主力净占比
        f124: prefix,
        // 新浪没有返回值的字段留空，不做任何估算
        f4: null,
...
async function handler24(req, res) {
  ...
  const list = (jsonData?.result?.cmsArticleWebOld || []).map((a) => ({
    title: a.title || "",
...
async function handler25(req, res) {
  ...
  const jsonData = stripJsonp(r);
  const list = (jsonData?.data?.list || []).map((a) => ({
    title: a.title || "",
...
async function handler27(req, res) {
  ...
  const upstreamReq = https2.request(options, (upstreamRes) => {
