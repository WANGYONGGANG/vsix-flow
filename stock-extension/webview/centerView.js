...
function renderStockDetail(s){
  ...
  if(sideToggle&&sidePanel){sidePanel.style.transition='width .2s ease,visibility .2s ease';sideToggle.addEventListener('click',function(){var collapsed=sidePanel.style.width==='0px';if(collapsed){sidePanel.style.width='150px';sidePanel.style.visibility='';sidePanel.style.borderLeft='';sidePanel.style.overflow='';sidePanel.style.padding='';sideToggle.textContent='▶'}else{sidePanel.style.width='0px';sidePanel.style.visibility='hidden';sidePanel.style.borderLeft='none';sidePanel.style.overflow='hidden';sidePanel.style.padding='0';sideToggle.textContent='◀'}var anim=0;function redrawLoop(){if(_klPeriod==='intraday'&&_intradayRedrawData)drawIntraday(_intradayRedrawData);else if(_kl.data.length)renderChart();anim=requestAnimationFrame(redrawLoop)}anim=requestAnimationFrame(redrawLoop);setTimeout(function(){cancelAnimationFrame(anim)},250)})}
...
window.addEventListener('message',function(e){
  ...
  if(msg.type==='tabData'&&msg.tab===currentTab){
    ...
    else if(_isFutures&&_klPeriod==='intraday'&&msg.period==='intraday'&&msg.data&&msg.data.length){
      drawIntraday({minutes:msg.data,preClose:Number(msg.data[0].split(',')[1]||0)});
    }
    else if(_isFutures&&_klPeriod!=='intraday'&&msg.period!=='intraday'){drawKline(msg.data||[]);}
    else if(!_isFutures){drawKline(msg.data||[]);}
