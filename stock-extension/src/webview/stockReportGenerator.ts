// 选股报告生成器：拉取行情/资金流/财务/股东数据，按六维加权模型计算得分，
// 生成与交付模板一致结构的完整报告 HTML（含雷达图 / K线 / 资金流 SVG）。
...
function upDownCls(v: number): string {
  const n = Number(v || 0);
...
function seg(v: number, s: [number, number][]): number {
  for (const [threshold, score] of s) {
...
function parseFinance(items: any[]): Record<string, string> {
  ...
  for (const it of items || []) {
    if (it && it.label) map[it.label] = String(it.value ?? '');
  }
  return map;
}
...
function calcMACD(closes: number[]): { dif: number; dea: number; hist: number } {
  const ema = (n: number): number => {
    let k = 2 / (n + 1), e = closes[0];
    for (let i = 1; i < closes.length; i++) e = closes[i] * k + e * (1 - k);
    return e;
  };
  const dif12 = ema(12), dif26 = ema(26);
  const dif = dif12 - dif26;
  // 简化 DEA：用近20根DIF近似 EMA9
  let k = 2 / 10, dea = dif;
  const hist = dif - dea;
  return { dif, dea, hist };
}
...
function parseKlineRows(rows: string[]): { closes: number[]; vols: number[] } {
  ...
  for (const row of rows) {
    ...
    if (p.length >= 6) {
      ...
      if (!isNaN(c)) { closes.push(c); vols.push(parseFloat(p[5]) || 0); }
    }
  }
  return { closes, vols };
}
...
function scoreMoney(it: RankItem, flow: FlowDay[]): number {
  ...
  let p5 = 50, p20 = 50;
  if (flow.length) {
    const last5 = flow.slice(-5), last20 = flow.slice(-20);
    p5 = (last5.filter((d) => d.main > 0).length / last5.length) * 100;
    p20 = (last20.filter((d) => d.main > 0).length / last20.length) * 100;
  }
...
function scoreTrend(it: RankItem, deep: DeepData): number {
  ...
  const price = it.f2 || 0;
  if (!closes.length || !price) return 50;
  let maAlign = 40;
  ...
  const sMacd = macdDif > macdDea ? (rising ? 95 : 80) : (macdDif > 0 ? 70 : (macdHist > macdHist ? 60 : 40));
...
function vols20(vols: number[], n: number): number[] {
  return vols.length ? vols.slice(-n) : [];
}
...
function scoreFinance(fin: Record<string, string>): number {
  const roe = numOf(fin['净资产收益率']);
  const margin = numOf(fin['净利率']);
  const rev = numOf(fin['营收同比增长']);
  const profit = numOf(fin['净利润同比增长']);
  const debt = numOf(fin['资产负债率']);
...
function scoreHolder(holders: HolderItem[]): number {
  const h0 = holders[0];
  if (!h0 || isNaN(h0.holderNumRatio)) return 50;
  let consec = 0;
  for (const h of holders) {
    if (h.holderNumRatio != null && h.holderNumRatio < 0) consec++;
    else break;
  }
...
function radarSvg(scores: number[]): string {
  ...
  const rings = [20.2, 40.5, 60.75, 81];
  let s = '';
  for (const r of rings) {
    s += `<polygon points="${angles.map((a) => pt(r, a).map((x) => x.toFixed(1)).join(',')).join(' ')}" fill="none" stroke="#dee2e6" stroke-width="1"></polygon>`;
  }
  for (const a of angles) {
    const p = pt(81, a);
    s += `<line x1="${cx.toFixed(1)}" y1="${cy.toFixed(1)}" x2="${p[0].toFixed(1)}" y2="${p[1].toFixed(1)}" stroke="#dee2e6" stroke-width="1"></line>`;
  }
  const dataPts = scores.map((sc, i) => {
    const r = 20.2 + sc * 0.6;
    return pt(r, angles[i]);
  });
  ...
  for (const p of dataPts) {
    s += `<circle cx="${p[0].toFixed(1)}" cy="${p[1].toFixed(1)}" r="3" fill="#e03131"></circle>`;
  }
  const labels = [
...
function flowSvg(flow: FlowDay[]): string {
  ...
  for (let i = 0; i < flow.length; i++) {
    ...
    const h = (Math.abs(v) / (maxAbs / 1e8)) * 38;
...
function dimBars(dims: ScoredStock['dims']): string {
  ...
  let h = '';
  for (let i = 0; i < 6; i++) {
    const v = Math.round(dims[keys[i]]);
...
function planHtml(it: RankItem, deep: DeepData): string {
  const price = it.f2 || 0;
  const ma5 = deep.ma5, ma20 = deep.ma20;
  if (!price) return '';
  ...
  return `<div class="plan">
    ...
    <div class="pitem"><span>盈亏比</span><b>${rrStr}</b></div>
  </div>`;
}
...
function moneyReason(it: RankItem, deep: DeepData): string[] {
  ...
  if (deep.flow.length) {
    ...
    const n20 = last20.filter((d) => d.main > 0).length;
...
function trendReason(it: RankItem, deep: DeepData): string[] {
  const price = it.f2 || 0;
  if (!deep.closes.length) return ['K线数据不足，走势按中性处理'];
...
function financeReason(fin: Record<string, string>): string[] {
  ...
  const margin = numOf(fin['净利率']);
  const debt = numOf(fin['资产负债率']);
  const rev = fin['营收同比增长'] || '-';
  const profit = fin['净利润同比增长'] || '-';
  const roeStr = isNaN(roe) ? '-' : roe.toFixed(1);
  const period = fin['报告期'] || '';
  return [
...
function holderReason(holders: HolderItem[]): string[] {
  const h0 = holders[0];
  if (!h0 || isNaN(h0.holderNumRatio)) return ['股东户数数据缺失，按中性处理'];
  const ratio = Number(h0.holderNumRatio);
...
function riskList(it: RankItem, deep: DeepData, sector: SectorItem | undefined, holders: HolderItem[]): string[] {
  const out: string[] = [];
  const price = it.f2 || 0;
  if (deep.closes.length) {
    const win20 = deep.closes.slice(-20);
    const max20 = Math.max(...win20);
    if (price >= max20 * 0.99) out.push('股价贴近20日高点，压力位附近换手需求大');
  }
  if ((it.f8 || 0) >= 15) out.push(`换手率 ${it.f8.toFixed(1)}% 过热，短期博弈激烈`);
  if ((it.f3 || 0) >= 9.8) out.push('当日涨停，次日高开幅度决定实际成本');
  const h0 = holders[0];
  if (h0 && !isNaN(h0.holderNumRatio) && Number(h0.holderNumRatio) > 0) out.push('股东户数明显增加，筹码趋于分散');
...
function cardHtml(st: ScoredStock, idx: number, sectorMap: Record<string, SectorItem>): string {
  const it = st, deep = st.deep, d = st.dims;
  const sector = sectorMap[st.f100];
  ...
  const hlText = hl.map(([n, sc]) => {
    ...
    if (n === '财务分析') {
      const roe = numOf(deep.fin['净资产收益率']);
      return `财务分析（${Math.round(sc)}分）：年化 ROE ≈ ${isNaN(roe) ? '-' : roe.toFixed(1)}%`;
    }
  ...
  const reasonBlocks = reasons.map((r) =>
    `<div class="rblock"><div class="rhead">${r.n}<span class="rscore" style="background:${scoreColor(Math.round(r.sc))}">${Math.round(r.sc)}</span></div><ul>${r.ls.map((li) => `<li>${li}</li>`).join('')}</ul></div>`
  ).join('');
  ...
  <div class="reasons">${reasonBlocks}</div>
...
function poolRowHtml(st: ScoredStock, hit: boolean, idx: number): string {
  const d = st.dims;
  const cls = (v: number) => ` style="color:${scoreColor(Math.round(v))}"`;
  const total = Math.round(st.total * 10) / 10;
...
function buildReportHtml(opts: {
  ...
  // 行业 map 用于卡片渲染：以名称匹配（f100 与板块名一致时生效）
...
body{margin:0;background:#f4f5f7;color:#212529;
...
.hero{background:linear-gradient(135deg,#fff 0%,#fff5f5 100%);border:1px solid #ffe3e3;
...
a:hover{text-decoration:underline;color:#e03131}
a[data-em]{cursor:pointer}
...
.fstep{flex:1;min-width:130px;background:#f8f9fa;border:1px solid #e9ecef;border-radius:10px;
...
.tbl th{background:#f8f9fa;color:#868e96;font-weight:600;font-size:12px;
  padding:9px 8px;text-align:left;border-bottom:2px solid #e9ecef;white-space:nowrap}
...
.pool tr.hit{background:#fff5f5}
.pool tr.hit .nm::after{content:"★";color:#e03131;margin-left:4px}
...
.card{background:#fff;border:1px solid #e9ecef;border-left:4px solid #e03131;
...
.crank{width:42px;height:42px;border-radius:50%;background:#e03131;color:#fff;
...
.theme{margin-top:12px;background:#fff9db;border:1px solid #ffec99;border-radius:8px;
  padding:8px 12px;font-size:12.5px;color:#856404}
.metrics{display:grid;grid-template-columns:repeat(8,1fr);gap:1px;background:#f1f3f5;
  border:1px solid #f1f3f5;border-radius:8px;overflow:hidden;margin:14px 0}
...
.plan{display:grid;grid-template-columns:repeat(3,1fr);gap:1px;background:#f1f3f5;
  border:1px solid #f1f3f5;border-radius:8px;overflow:hidden}
...
.divnote{background:#e7f5ff;border:1px solid #a5d8ff;border-radius:10px;padding:12px 16px;
  margin-bottom:18px;font-size:12.5px;color:#1864ab}
.divnote ul{margin:6px 0 0;padding-left:18px} .divnote li{margin:2px 0}
.method{font-size:13px;color:#495057}
.method table{width:100%;border-collapse:collapse;margin:10px 0;font-size:12.5px}
.method td,.method th{border:1px solid #e9ecef;padding:7px 10px;text-align:left}
.method th{background:#f8f9fa}
.disc{background:#f8f9fa;border:1px dashed #ced4da;border-radius:10px;padding:14px 18px;
  color:#868e96;font-size:12.5px;line-height:1.8}
@media(max-width:900px){.cbody{grid-template-columns:1fr}.reasons{grid-template-columns:1fr}
...
<section><h2>阶段一 · 全市场主力资金净流入 TOP 20</h2>
  <p class="sub" style="margin-top:-6px;margin-bottom:12px">资金是行情的先行指标。
  以东财实时资金流为口径，按当日主力（大单+超大单）净流入金额排序，
  并剔除 ST 股、一字板封死股、市值低于 25 亿的微盘股。</p>
...
<section><h2>阶段二 · 行业板块异动</h2>
  <p class="sub" style="margin-top:-6px;margin-bottom:14px">
  个股的资金流入若无板块效应支撑，往往是孤立行为、持续性弱。
  这一步核对候选股所属行业的资金流向、涨跌幅与赚钱效应（上涨家数占比）。</p>
...
<section><h2>阶段三 / 四 · 最可能继续上涨的 ${hits.length} 只个股</h2>
  <p class="sub" style="margin-top:-6px;margin-bottom:16px">
  对入围个股逐一执行基本面（估值与市值结构）、股东分析（筹码集中度）、
  财务分析（盈利能力与成长性）、走势分析（均线/量价/超买）四维检验，
  与资金面、行业异动一起加权得出综合得分。</p>
  ${cards2}
</section>

<section><h2>候选池完整评分（${pool.length} 只）</h2>
  <p class="sub" style="margin-top:-6px;margin-bottom:12px">带 ★ 者为最终入选标的。
  各维度分数经权重加权后得到综合分，可据此复核模型的每一步判断。</p>
...
<p>本报告采用<b>六维加权评分模型</b>，每一维度均由分段阈值函数映射到 0~100 分，
避免单一极端值主导排序；数据缺失的维度按中性 50 分处理，不奖励也不惩罚。</p>
<table><tbody><tr><th>维度</th><th>权重</th><th>核心指标</th><th>逻辑</th></tr>
<tr><td>资金面</td><td>25%</td><td>主力净流入额、净占成交比、超大单结构、近5/20日持续性</td>
<td>资金是短期行情的直接驱动力，且要求「持续」而非单日脉冲</td></tr>
<tr><td>走势分析</td><td>25%</td><td>均线多头排列、20日区间位置、量能比、RSI、MACD</td>
<td>趋势向上且未过度透支的标的，惯性延续概率更高</td></tr>
<tr><td>行业异动</td><td>15%</td><td>板块涨幅、板块主力净流入、上涨家数占比</td>
<td>板块效应决定了个股上涨的持续性与容错空间</td></tr>
<tr><td>财务分析</td><td>13%</td><td>ROE、销售净利率、营收与净利同比、资产负债率</td>
<td>业绩是股价上涨的地基，规避纯炒作标的</td></tr>
<tr><td>基本面</td><td>12%</td><td>PE(TTM)、PB、总市值、换手率</td>
<td>估值决定安全边际与后续空间</td></tr>
<tr><td>股东分析</td><td>10%</td><td>股东户数环比、连续下降期数、户均持股</td>
<td>户数下降=筹码集中=主力吸筹，是中线上涨的前置条件</td></tr></tbody></table>
<p><b>数据来源：</b>东方财富（资金流 / 行业板块 / 股东户数）、腾讯财经（实时行情 / 前复权日K）、
新浪财经（利润表）、通达信（财务快照）。全部为公开接口直连。</p>
</div></section>

<div class="disc">
<b>免责声明</b><br>
本报告由量化模型基于公开市场数据自动生成，所有评分、买卖区间与目标位均为模型计算结果，
不构成任何投资建议或买卖要约。模型无法预测突发消息、政策变化与系统性风险，
历史资金流与技术形态亦不代表未来表现。股市有风险，入市需谨慎，据此操作，风险自担。<br>
报告生成于 ${opts.generatedAt}，数据具有时效性，盘中数据会随行情实时变动。
</div>
</div>`;
}

// 过滤规则：ST / 退 / 一字板（涨幅>=9.8 且换手<0.5%）/ 微盘（市值<25亿）
...
export async function generateStockReport(fetchFn: FetchFn): Promise<string> {
  const t0 = Date.now();
  const now = new Date();
  ...
  for (const name of idxNames) {
    const m = (ov?.data?.diff || []).find((d: any) => d.f14 === name);
    if (m) indices.push(m);
  }
  ...
  const sectors: SectorItem[] = (sec?.data?.diff || []).map((d: any) => ({
    f12: d.f12, f14: d.f14, f3: d.f3 || 0, f62: d.f62 || 0,
    f104: d.f104 || 0, f105: d.f105 || 0, f204: d.f204 || '', f205: d.f205 || '',
  }));
  const concepts: any[] = con?.data?.list || [];

  // 初筛：资金 60% + 行业 40%，取前 15 深度分析
  const sectorMap: Record<string, SectorItem> = {};
  for (const s of sectors) sectorMap[s.f14] = s;
  const rankByFlow = filtered.slice().sort((a, b) => (b.f62 || 0) - (a.f62 || 0));
  const maxFlow = Math.max(...rankByFlow.map((r) => r.f62 || 0), 1);
  const scoredPre = rankByFlow.map((it) => {
    const sMoney = seg((it.f62 || 0) / 1e8, [[15, 100], [10, 90], [5, 80], [2, 65], [0, 50], [-99, 30]]);
    const secItem = sectorMap[it.f100];
    const sInd = scoreIndustry(secItem);
    return { it, sMoney, sInd };
  });
  const candidates = scoredPre.sort((a, b) => (0.6 * b.sMoney + 0.4 * b.sInd) - (0.6 * a.sMoney + 0.4 * a.sInd)).slice(0, 15);

  // 深度分析（并行拉取 K线/资金流/财务/股东）
  ...
  const pool: ScoredStock[] = deepResults.map(({ it, deep }) => {
    const secItem = sectorMap[it.f100];
    const dims = {
      money: scoreMoney(it, deep.flow),
      trend: scoreTrend(it, deep),
      industry: scoreIndustry(secItem),
      finance: scoreFinance(deep.fin),
      basic: scoreBasic(it),
      holder: scoreHolder(deep.holders),
    };
